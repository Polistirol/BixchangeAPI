...
def register_request(request):
	if request.method == "POST":
		...
		if form.is_valid():
			...
			message = svc.validate_referral(user) #checks if referral code is valid and distribute the rewards
			user.save()
		...
		if action=="login":
			...
			if not request.user.is_authenticated:
				...
				if action =="balance":
					...
					if price: return JsonResponse({"action": "get_btc_price","succes":price,"log":""},safe = False)
					else: return JsonResponse({"action": "get_btc_price","succes":None,"log":"Failed to fetch price"},safe = False)	
	else:
