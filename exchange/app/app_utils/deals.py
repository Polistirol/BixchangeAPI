    ...
    def getFinalPrice(self,askedPrice,targetAmount,orderType):
        if self.orderList:
            for order in self.orderList:
                self.finalPrice += abs(((order.USDprice + askedPrice ) /2 )* order.amount)
            saved = abs(askedPrice*targetAmount - self.finalPrice)
            if orderType == "buy":
                msg.ok(f"Final price is {self.finalPrice}! \n{saved}$ saved from {askedPrice*targetAmount}$ asked!\n")
            else:
                msg.ok(f"Final price is {self.finalPrice}! \n{saved}$ extra earned from {askedPrice*targetAmount}$ asked!\n")
            return self
        else: msg.error("Order list not found while getting final price")
        

def subset_sum_orders(orders: list, target, partial=[], partial_sum=0,orderlist=[]):
    ...
    for index, order in enumerate(orders):
        ...
        yield from subset_sum_orders(remaining, target, partial + [order], partial_sum + order.amount)
...
def get_backup(ordersFull,targetAmount,orderType):
    print("need backup")
    newBestDeal = []
    i=1
    while not newBestDeal:
        print("recycling with target =",targetAmount- i)
        generator = subset_sum_orders(ordersFull,targetAmount-i)
...
def find_best_deal(newOrder ,ordersFull : list,maxLenOrder:int = 50) -> list:
    ...
    if ordersFull:
        ...
        if bestDeal:
           ...
           bestDeal = get_backup(ordersFull,targetAmount,orderType)
        return bestDeal
