...
class msg:
    ...
    def no_funds(order):
        msg.error(
            f"You dont have sufficent funds to cover your order at the moment\n")
        msg.info(
            f"But the order is now live in the market. Order ID :{order.id} ")

    def exchange_uncover(order):
        a = msg.wrn(
            f"The Exchange can't entirely fulfill this order at the moment..")
        b = msg.info(
            f"The order is now live in the market. Order ID :{order.id} ")
        return [a, b]
