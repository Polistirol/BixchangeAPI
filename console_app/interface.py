...
def single_order_options(ID, status):
    while True:
        if status == "Open":
            r = input(
                "Press C to [C]ancel this order - [X] to go back  ").strip().lower()
            if r == "c":
...
def get_new_order_data(btcPrice):
    ...
    if orderType not in ["1", "2"]:
        display_btc_market_price(btcPrice)
    while True:  # get price
        if orderType not in ["1", "2"]:
            ...
            break
    msg.wrn(
        f"New order of type: {OrderType.typeDict[int(orderType)]} for {amount} BTC at {USDprice}$ each\n CONFIRM? [Y]es\[N]o ")
    confirmed = input("").lower().strip()
    if confirmed == "y":
